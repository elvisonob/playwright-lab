...
export class DatepickerPage {
  ...
  async selectDatePickerWithRangeFromToday(
    ...
    await calendarInputField.click();
    const expectedDateStart =
      await this.selectDateInTheCalendar(daysFromTodayStart);
    const expectedDateEnd =
      await this.selectDateInTheCalendar(daysFromTodayEnd);
  ...
  private async selectDateInTheCalendar(daysFromToday: number) {
    ...
    while (!currentMonthAndYear?.includes(expectedMonthAndYear)) {
      ...
      currentMonthAndYear = await this.page
        .locator('nb-calendar-view-mode')
        .textContent();
    }
